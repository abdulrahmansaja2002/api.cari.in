...
class VBEPostings:
    ...
    def encode(postings_list):
        ...
        gap_based_list = [
            postings_list[i]
            if i == 0 
            else postings_list[i] - postings_list[i-1]
    ...
    def decode(encoded_postings_list):
        ...
        decoded_postings_list.frombytes(encoded_postings_list)
        decoded_postings_list = VBEPostings.vb_decode(encoded_bytestream=decoded_postings_list)
        ungap_based_list = []
        for i in range(len(decoded_postings_list)):
            if i == 0:
                ungap_based_list.append(decoded_postings_list[i])
            else:
                ungap_based_list.append(decoded_postings_list[i] + ungap_based_list[i-1])
        return ungap_based_list
