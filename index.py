...
class InvertedIndex:
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            self.postings_dict, self.terms, self.doc_length, self.avdl = pickle.load(f)
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(self.metadata_file_path, 'wb') as f:
            avdl = sum(self.doc_length.values()) / len(self.doc_length)
            pickle.dump([self.postings_dict, self.terms, self.doc_length, avdl], f)

    
    def count_all_term_upper_bound_score(self, k1=1.2, b=0.75):
        """Count upper bound score for all term in index
            - tf = term frequency
            ...
            - TF-IDF
                * tfidf = idf * (1 + log10(tf))

            - BM25
                * bm25 = idf * (k1 + 1) * tf
                * bm25 /= k1 * ((1 - b) + b * dl / avdl) + tf
        
        Args:
            k1 (float, optional): A BM25 scoring parameter (k1) .Defaults to 1.2.
            b (float, optional): A BM25 scoring parameter (b) Defaults to 0.75.
        """

        def count_upper_bound_score_bm25(postings_list, tf_list):
            N = len(self.doc_length)
            df = len(postings_list)
            idf = math.log10(N/df)
            max_score = 0
            for doc, tf in zip(postings_list, tf_list):
                dl = self.doc_length[doc]
                score = idf * (k1 + 1) * tf
                score /= k1 * ((1 - b) + b * dl / self.avdl) + tf
                max_score = max(max_score, score)
            return max_score
        def update_term_upper_bound_score(term, bm25, tfidf):
            if len(self.postings_dict[term]) == 6:
                self.postings_dict[term] = self.postings_dict[term][:4] + (bm25, tfidf)
            else:
                self.postings_dict[term] += (bm25, tfidf)
        def count_upper_bound_score_tfidf(postings_list, tf_list):
            N = len(self.doc_length)
            df = len(postings_list)
            idf = math.log10(N/df)
            max_score = 0
        ...
        for term in self.postings_dict:
            ...
            tf_list = self.get_tf_list(term)
            if len(tf_list) == 0:
                continue
            max_score_bm25 = count_upper_bound_score_bm25(postings_list, tf_list)
            max_score_tfidf = count_upper_bound_score_tfidf(postings_list, tf_list)

            update_term_upper_bound_score(term, max_score_bm25, max_score_tfidf)
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        if term in self.postings_dict:
            if DEBUG: print("DEBUG | get_postings_list()", term, self.postings_dict[term])
            start_position_in_index_file = self.postings_dict[term][0]
            number_of_postings_in_list = self.postings_dict[term][1]
            length_in_bytes_of_postings_list = self.postings_dict[term][2]
            # self.index_file.seek(0)
            self.index_file.seek(start_position_in_index_file)
            postings_list = self.index_file.read(length_in_bytes_of_postings_list)
            return self.postings_encoding.decode(postings_list)
        return []
    def get_tf_list(self, term):
        if term in self.postings_dict:
            if DEBUG: print("DEBUG | get_tf_list()", term, self.postings_dict[term])
            start_position_in_index_file = self.postings_dict[term][0]
            length_in_bytes_of_postings_list = self.postings_dict[term][2]
            length_in_bytes_of_tf_list = self.postings_dict[term][3]

            # self.index_file.seek(0)
            self.index_file.seek(start_position_in_index_file + length_in_bytes_of_postings_list)
            tf_list = self.index_file.read(length_in_bytes_of_tf_list)
            return self.postings_encoding.decode_tf(tf_list)
        return []
    def get_upper_bound_score(self, term, scoring='bm25'):
        if term in self.postings_dict:
            if scoring == 'bm25':
                ...
                raise Exception("scoring must be 'bm25' or 'tfidf'")
        return 0
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        # get start_position_in_index_file (the end of index_file)
        start_position_in_index_file = self.index_file.seek(0,2)
        # get number_of_postings_in_list
        number_of_postings_in_list = len(postings_list)
        # get length_in_bytes_of_postings_list
        length_in_bytes_of_postings_list = len(encoded_postings_list)
        length_in_bytes_of_tf_list = len(encoded_tf_list)
        # Save metadata
        self.terms.append(term)
        for doc, tf in zip(postings_list, tf_list):
            self.doc_length[doc] = self.doc_length.get(doc, 0) + tf
        # append term, start_position_in_index_file, number_of_postings_in_list, length_in_bytes_of_postings_list to postings_dict
        ...
        if DEBUG: print(f"index.append() | posting dict[{term}]: {self.postings_dict[term]}")
        # append encoded_postings_list to index_file
        self.index_file.write(encoded_postings_list)
        # append encoded_tf_list to index_file
        self.index_file.write(encoded_tf_list)
        # update the index_file postion
        self.index_file.seek(0,2) # 2 = end of file (EOF)
        # append term to terms
        # print("DEBUG |", term, postings_list)
        return []
